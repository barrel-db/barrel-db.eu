...
description: Expose a barrel database over HTTP/1.1 and HTTP/2 with barrel_server, including endpoints, auth, and CORS.
---
...
both kinds. See [spaces](/docs/server/spaces), [mcp](/docs/server/mcp), and
[barrel-lite](/docs/guides/barrel-lite).
...
its own origin policy. See [barrel-lite](/docs/guides/barrel-lite).
...
- Replication over the wire ships today (the `/db/:db/_sync/*` endpoints; see
  [synchronization](/docs/guides/synchronization)). gRPC, HTTP/3, WebTransport, a
