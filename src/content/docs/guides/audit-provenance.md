...
description: Attribute every write to an agent, session, and source, and read past versions of a document within the retention window.
---
...
is `mcp`. See the [MCP guide](/docs/server/mcp).
...
- Branches fork with the parent's history, so a branch carries the
  provenance of everything before the fork (see [timeline](/docs/guides/timeline)).
