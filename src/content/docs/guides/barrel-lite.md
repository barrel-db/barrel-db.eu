---
title: barrel-lite (browser client)
description: A TypeScript client that keeps an offline-first document store in the browser and syncs it to a barrel_server.
---
...
instead (see [spaces](/docs/server/spaces)); it authenticates only that space's `/db`
